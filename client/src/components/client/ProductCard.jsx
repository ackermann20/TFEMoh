...
const ProductCard = ({ produit }) => {
  ...
  // Obtenir la couleur d'arrière-plan selon le type de produit
  ...
  return (
    <div 
      className={`bg-white rounded-lg overflow-hidden shadow-md hover:shadow-xl transition-all duration-300 w-full max-w-xs ${isHovered ? 'transform scale-[1.02]' : ''}`}
    ...
    >
      <div className="relative">
        ...
        <div className={`absolute top-2 right-2 px-2 py-1 rounded-full text-xs font-medium ${getTypeBadgeColor(produit.type)}`}>
          {produit.type ? produit.type.charAt(0).toUpperCase() + produit.type.slice(1) : 'Produit'}
        </div>
      </div>

      <div className="p-4">
        <h3 className="text-lg font-bold text-amber-800 mb-1">{produit.nom}</h3>
        <p className="text-gray-600 text-sm h-12 overflow-hidden mb-3">
          {produit.description || "Produit frais de notre boulangerie"}
        </p>
        ...
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className={`relative overflow-hidden ${
              produit.type === 'sandwich' ? 'bg-blue-600 hover:bg-blue-700' : 'bg-amber-600 hover:bg-amber-700'
